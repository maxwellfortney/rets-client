...
import { RetsProcessingError, RetsReplyError } from '../models';
...
export async function parseRetsResponse(source, recordXmlTagName, ignoreRepliedError = false) {
    const document = await parseString(source).catch((e) => e);
    if (document instanceof Error) {
        throw new RetsProcessingError(document);
    }
    if (!document.RETS) {
        throw new RetsProcessingError(new TypeError('Unable to find RETS root element'));
    }
    ...
    if (root['COLUMNS']) { // COMPACT数据
        let dataDelimiter = '\t';
        if (root['DELIMITER']) {
            dataDelimiter = decodeHexString(root['DELIMITER'][XML_ATTRIBUTES].value);
        }
